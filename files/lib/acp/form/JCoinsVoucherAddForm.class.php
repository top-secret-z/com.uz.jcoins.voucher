...
class JCoinsVoucherAddForm extends AbstractForm
{
    ...
    public function readFormParameters()
    {
        ...
        if (isset($_POST['jCoins'])) {
            $this->jCoins = \intval($_POST['jCoins']);
        }
        if (isset($_POST['period'])) {
            $this->period = \intval($_POST['period']);
        }
        if (isset($_POST['periodUnit'])) {
            $this->periodUnit = StringUtil::trim($_POST['periodUnit']);
        }

        if (isset($_POST['redeemLimit'])) {
            $this->redeemLimit = \intval($_POST['redeemLimit']);
        }
        if (isset($_POST['codeRedeemLimit'])) {
            $this->codeRedeemLimit = \intval($_POST['codeRedeemLimit']);
        }
        if (isset($_POST['codeUserLimit'])) {
            $this->codeUserLimit = \intval($_POST['codeUserLimit']);
        }
        if (isset($_POST['codePrefix'])) {
            $this->codePrefix = StringUtil::trim($_POST['codePrefix']);
        }
        if (isset($_POST['codeNumber'])) {
            $this->codeNumber = \intval($_POST['codeNumber']);
        }
        ...
        if (isset($_POST['footer']) && \is_array($_POST['footer'])) {
            $this->footer = ArrayUtil::trim($_POST['footer']);
        }
        ...
        if (isset($_POST['notify'])) {
            $this->notify = \intval($_POST['notify']);
        }
    ...
    public function save()
    {
        ...
        if ($voucher->typeDes == 'code') {
            ...
            for ($i = 0; $i < $voucher->codeNumber; $i++) {
                ...
                if ($voucher->codePrefix != '') {
                    $code = $voucher->codePrefix . '-' . $code;
                }
    ...
    public function validate()
    {
        ...
        if (empty($this->title)) {
            throw new UserInputException('title', 'required');
        }
        ...
        if ($this->type->typeTitle == 'code') {
            ...
            // max length
            if (\mb_strlen($this->codePrefix) > 5) {
                throw new UserInputException('codePrefix', 'tooLong');
            }
        }
