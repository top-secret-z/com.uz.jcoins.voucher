...
class JCoinsVoucher extends DatabaseObject implements IRouteController {
	...
	public function getSubject($acp = 0) {
		if ($this->getItemContent() !== null) {
			if ($acp) {
				...
				$content = $this->getItemContent()->getSubject();
				return $content;
			}
	...
	public function getBBCodeSubject() {
		if ($this->getItemContent() !== null) {
			if (!empty($this->getItemContent()->getSubject())) $test = $this->getItemContent()->getSubject();
			else $test = $this->getItemContent()->getFormattedContent();
	...
	public function getContent($acp = 0) {
		if ($this->getItemContent() !== null) {
			if ($acp) {
				...
				$content = $this->getItemContent()->getFormattedContent();
				return $content;
			}
	...
	public function getFooter($acp = 0) {
		if ($this->getItemContent() !== null) {
			if ($acp) {
				...
				$content = $this->getItemContent()->getFooter();
				return $content;
			}
	...
	public function hasRedeemed() {
		...
		return (isset(self::$redeemCache[$this->voucherID]) ? self::$redeemCache[$this->voucherID] : 0);
	}
	...
	public function hasRedeemedTimes() {
		...
		return (isset(self::$redeemTimesCache[$this->voucherID]) ? self::$redeemTimesCache[$this->voucherID] : 0);
	}
	...
	public function canRedeem() {
		...
		if (!WCF::getSession()->getPermission('user.jcoins.voucher.canSee')) return false;
		
		// limits
		if ($this->expirationStatus && $this->expirationDate < TIME_NOW) return false;
		if ($this->isExpired) return false;
		if ($this->redeemLimit > 0 && $this->redeemed >= $this->redeemLimit) return false;
		
		if ($this->typeDes == 'code') {
			if ($this->codeNumber * $this->codeRedeemLimit <= $this->redeemed) return false;
			if ($this->userRedeemedTimes() >= $this->codeUserLimit) return false;
		}
		else {
			if ($this->hasRedeemed()) return false;
		}
	...
	public static function getVouchers() {
		...
		$voucherIDs = array_merge($voucherIDs, self::getCodeVouchers());
	...
	public static function getCodeVouchers() {
		...
		if (!empty($vouchers)) {
			foreach ($vouchers as $voucher) {
				...
				if (!empty($users)) $voucherIDs[] = $voucher->voucherID;
			}
		}
	...
	public function codeExists($code) {
		...
		$statement = WCF::getDB()->prepareStatement($sql, 1);
		$statement->execute([$code, $this->voucherID]);
		if ($row = $statement->fetchArray()) return $row['redeemed'];
		return null;
	}
}
